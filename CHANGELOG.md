...
## [0.8.12-gpthub.9] - 2026-04-12

### Added

- 🤖 **Интеллектуальный авторутинг запросов (`auto` модель).** Добавлен новый модуль `backend/open_webui/utils/auto_routing.py`, реализующий автоматический выбор наиболее подходящей AI-модели на основе анализа текста запроса и прикреплённых файлов.

  **Как это работает:**
  1. Пользователь выбирает модель `Auto (Intelligent Routing)` в интерфейсе.
  2. При каждом новом сообщении backend анализирует последнее сообщение пользователя с помощью набора regex-паттернов.
  3. На основе результата запрос маршрутизируется к специализированной модели:
     - 🎨 `black-forest-labs/flux.2-max` — генерация изображений (паттерн: "нарисуй", "draw", "generate image" и др.)
     - 🎵 `google/lyria-3-pro-preview` — генерация аудио/музыки ("звук", "мелодия", "song" и др.)
     - 👁️ `google/gemini-3.1-flash-lite-preview` — анализ изображений (когда к сообщению прикреплено фото)
     - 💻 `qwen/qwen3.6-plus` — код, математика, аналитика, исследования, документы, творческий текст (fallback)
  4. Виртуальная модель `auto` инжектируется в кэш моделей (`OPENAI_MODELS`) как внутренняя, что позволяет избежать лишних запросов к провайдерам.
  5. Прикреплённые файлы (изображения, аудио, документы) автоматически препроцессируются: изображения конвертируются в base64 и встраиваются в сообщение, аудио транскрибируется, текстовые файлы читаются и инжектируются в контекст.

### Fixed

- 🧩 **"Chunk too big" для моделей генерации изображений.** Flux и Lyria возвращают изображение/аудио как огромный base64-чанк внутри SSE-стрима, даже если запрошен `stream=false`. `stream_chunks_handler` урезал его до `data: {}`, что фронтенд показывал как "Chunk too big". Исправлено: перед `json.dumps` сохраняется флаг `stream_explicitly_disabled`, и при получении SSE-ответа от image/audio-модели тело читается **построчно** (без ожидания EOF) с остановкой по `data: [DONE]`, результат возвращается как обычный `JSONResponse`, минуя стрим-обработчик.

- ⏳ **Зависание ответа на несколько минут.** Предыдущая попытка фикса использовала `await r.json()` / `await r.text()`, которые ждут закрытия соединения (EOF). SSE-серверы часто держат соединение открытым бессрочно — отсюда зависание. Заменено на асинхронную итерацию по `r.content` построчно с `break` по `data: [DONE]`.
