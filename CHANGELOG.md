...
- **Recent Spaces Grid**: Заменены абстрактные приветственные карточки на Главной странице (Home Hub) сеткой последних рабочих пространств. Пользователи теперь могут переходить напрямую к активным проектам со стартового экрана, что обеспечивает лучшую связность UX между "Глобальным режимом" и "Режимом пространства".

### Changed

- **Global Sidebar Redesign**: Проведен редизайн внутреннего (правого) сайдбара на Главной странице. Удален дублирующийся логотип Vibe Hub, а раздел «Рабочее пространство» (Workspace) переименован в «Настройки ИИ» во избежание путаницы со сценариями Пространств.
- **Context-Aware Sidebar Visibility**: Внутренний сайдбар (с историей глобальных чатов) теперь полностью скрывается, когда пользователь находится внутри Конкретного Пространства, оставляя чистый фокус на самом пространстве.

### Fixed

- **Sidebar Reactivity Error**: Исправлена синтаксическая ошибка Svelte "Unexpected block closing tag", возникавшая из-за неверной вложенности `<div>` после применения новых стилей сайдбара.
- **Missing Module Import**: Исправлена ошибка сборки на Главной странице путем добавления недостающего импорта утилиты `getSpaceEmoji` в `Placeholder.svelte`.

## [0.8.12-gpthub.28] - 2026-04-21

### Added

- **Multi-tier Workspace Navigation**: Внедрена двухколоночная навигация (`SpacesColumn.svelte`). При выборе рабочего пространства боковая панель автоматически очищается от глобальных элементов и отображает чаты только для текущего контекста пространства.
- **Space Dashboard Overlay**: Экран аналитики пространства теперь интегрирован в `Placeholder.svelte` и переопределяет стандартное приветствие, когда пользователь находится внутри рабочего пространства.
- **Context-Aware Chat Creation**: Изолированный стор `$selectedProjectId` управляет маршрутизацией создания новых сессий — новые чаты, созданные внутри пространства, автоматически привязываются к нему, а фильтрация в сайдбаре работает мгновенно через реактивность Svelte.

### Fixed

- **Mem0 JSON Extraction Bug**: Обновлен `custom_fact_extraction_prompt` в модуле `mem0_manager.py` для явного извлечения данных в формате `{"facts": [...]}`. Это исправило критическую ошибку TypeError (`list indices must be integers or slices, not str`), которая блокировала запись новых фактов в Qdrant при инициализации памяти пространства.
- **Store Reactivity Cycle**: Убран избыточный системный редирект `goto('/')` при выборе пространства. Теперь переключение контекстов происходит без полной перезагрузки компонентов (spa-mounting), сохраняя HMR и избегая сброса стейта в сайдбаре.
...
- Синхронизация `package.json` с тегом релиза `v0.8.12-gpthub.24` (проверка CI `check-version-tag`).
...
- **Inline prompt hints** (`InlinePromptHints`): локальный fuzzy-поиск по заготовкам, debounced LLM-продолжение строки, кэш и без глобального троттлинга в `generateAutoCompletion`.
- **Task router selector** в workspace: режимы Авто / Чат / Код / Поиск / Фото (`type="button"` у кнопок).

### Changed

- Автодополнение: убран глобальный интервал между запросами; остаются кэш по ключу и dedupe inflight.
- Чат UI: подсказки, статусы ответа, цитаты, сайдбар и связанные экраны — правки под новый UX.

### Fixed

- LLM-подсказки больше не «глушатся» постоянным возвратом пустой строки при наборе текста.
...
-  **Changed RAG model** Изменена модель с локальной на MTS WEB.
